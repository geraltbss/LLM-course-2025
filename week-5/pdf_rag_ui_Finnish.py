...
if not get_from_session(st, SESSION_VARS.LOADED_MODELS):
    ...
    st.write("Mallit oli jo ladattu")

print_session(st)

st.title('Finnish PDF RAG-käyttöliittymä')
query = st.text_input(
    "Kirjoita kysymyksesi tähän",
    "Mitkä ovat perusopetuksen keskeiset tavoitteet?"
)
...
uploaded_file = st.file_uploader(
    label="Lataa PDF",
    help="Lataa suomenkielinen PDF RAG-hakua varten",
    type="pdf"
)

button_clicked = st.button("Luo")

if uploaded_file is not None:
    print(f"Ladattu tiedosto: {uploaded_file}")
    ...
    if not get_from_session(st, SESSION_VARS.PROCESSED_DATA):
        with st.expander("Esikäsittely"):
            st.write("Luetaan PDF")
            ...
            st.write("Tallennetaan upotukset")
    ...
    if get_from_session(st, SESSION_VARS.PROCESSED_DATA):
        ...
        for i, item in enumerate(context_items):
            item["pisteet"] = scores[i].cpu()  # return score back to CPU
        st.write(f"Kysely: {query}")
        with st.expander("Hakutulokset"):
            for score, index in zip(scores, indices):
                st.write(f"Pisteet: {score:.4f}")
                st.write(pages_and_chunks[index]["sentence_chunk"])
                st.write(f"Sivu: {pages_and_chunks[index]['page_number']}")
        with st.expander("Tulokset"):
            ...
            for score, index in zip(scores, indices):
                ...
                st.write(f"Sivun Numero: {pages_and_chunks[index]['page_number']}")

        st.write("Valitsit:", gen_variant)
        with st.expander(f"Vastaus kyselyyn: {query}"):
